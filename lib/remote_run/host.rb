class Host
  ...
  def copy_codebase
    ...
    if system(%{rsync --delete --delete-before --rsh='ssh #{SSH_CONFIG}' --delete-excluded #{excludes.join(" ")} --timeout=60 -a #{$runner.temp_path}/ #{ssh_host_and_user}:#{$runner.remote_path}/})
